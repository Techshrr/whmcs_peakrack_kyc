...
- `lib/Providers/`: provider interface, available Tencent/Alipay/manual providers, and reserved provider adapters.
...
The v1.2 development branch keeps the provider framework and adds database-backed OAuth state storage for Alipay callbacks, so callback validation does not depend only on PHP session state.
